#!/usr/bin/env python3
# Run: python3 test-a-4-plotters.py

"""
test-a-4-plotters.py

Connect to four AxiDraw units in interactive mode and draw the same 1 inch square.

Optional:
  python3 test-a-4-plotters.py --port1 "AxiDraw One" --port2 "AxiDraw Two" --port3 "AxiDraw Three" --port4 "AxiDraw Four"
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Connect four AxiDraw units and draw the same 1 inch square."
    )
    parser.add_argument(
        "--port1",
        type=str,
...
def main() -> int:
    ...
    try:
        ...
        print(f"Using plotter 4: {port_4_arg}")
        ...
        plotter_4 = build_plotter(port_4_arg, args.speed)
        connected_ports = [
            connected_port_name(plotter_1),
            connected_port_name(plotter_2),
            connected_port_name(plotter_3),
            connected_port_name(plotter_4),
        ]

        if len(set(connected_ports)) != 4:
            print("Connections resolved to fewer than four unique AxiDraw ports.")
            print("Pass --port1, --port2, --port3, and --port4 to target distinct machines.")
            return 1
