#!/usr/bin/env python3
# Run: python3 test-a-2-plotters.py

"""
test-a-2-plotters.py

Connect to two AxiDraw units in interactive mode and draw the same 1 inch square.

Optional:
  python3 test-a-2-plotters.py --port1 "AxiDraw One" --port2 "AxiDraw Two"
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Connect two AxiDraw units and draw the same 1 inch square."
    )
    parser.add_argument(
        "--port1",
        type=str,
...
def main() -> int:
    ...
    try:
        ...
        port_1_name = connected_port_name(plotter_1)
        port_2_name = connected_port_name(plotter_2)

        if port_1_name == port_2_name:
            print("Both connections resolved to the same AxiDraw port.")
            print("Pass --port1 and --port2 to target two distinct machines.")
            return 1
