#!/usr/bin/env python3
# Run: python3 test-d-1-plotter-dynamickolam.py

"""
test-d-1-plotter-dynamickolam.py

Generate one DFS-based kolam (matching the p5.js logic), show a preview window,
and draw that exact pattern on one AxiDraw.
...
def preview_pattern(pattern: KolamPattern, title: str) -> None:
    if tk is None:
        print("Tkinter is not available; skipping preview window.")
        return

    margin = 24
    width = int(math.ceil(pattern.width_px + margin * 2))
    height = int(math.ceil(pattern.height_px + margin * 2))
    ...
    for command in pattern.commands:
        if command.kind == "line" and command.line is not None:
            ...
            for x, y in pts:
                flat_points.append(x + margin)
                flat_points.append(y + margin)
            canvas.create_line(*flat_points, fill="#0048FF", width=1)

    label = tk.Label(
        root,
        text="Close this window to continue to plotting.",
        anchor="w",
        padx=8,
        pady=6,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate one dynamic DFS kolam, preview it in a window, "
            "then draw the same result on one AxiDraw."
        )
    )
    parser.add_argument("--size", type=int, default=5, help="Grid size (default: 10).")
    parser.add_argument("--spacing", type=float, default=50.0, help="Dot spacing in px (default: 50).")
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducible kolam.")
    parser.add_argument(
        "--pixels-per-inch",
        type=float,
        default=100.0,
    ...
    parser.add_argument(
        "--preview-only",
        action="store_true",
        help="Generate + preview only; do not connect to AxiDraw.",
    )
    parser.add_argument(
        "--no-preview",
        action="store_true",
...
def main() -> int:
    ...
    generator = DFSKolamGenerator(size=args.size, spacing=args.spacing, rng=rng)
    pattern = generator.generate()

    print(f"Generated kolam seed: {seed}")
    ...
    if not args.no_preview:
        preview_title = f"Dynamic Kolam Preview (seed {seed})"
        preview_pattern(pattern, preview_title)

    if args.preview_only:
        print("Preview-only mode enabled. Exiting without plotting.")
        return 0
    ...
    try:
        ...
        print("Kolam complete. Returned to home (0,0).")
        return 0
