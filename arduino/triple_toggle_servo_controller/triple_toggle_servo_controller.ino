...
constexpr uint8_t PLOTTER_COUNT = 3;

// Change these pins only if your wiring differs.
constexpr int SERVO_PINS[PLOTTER_COUNT] = {8, 9, 10};
...
bool atMarker[PLOTTER_COUNT] = {true, true, true};
...
void handleCommand(const String& rawCommand) {
  ...
  if (command.length() == 2 && command[0] == 't' && isDigit(command[1])) {
  ...
  Serial.println("OK COMMANDS: t1 t2 t3 s");
}
...
bool tryHandleCompactCommand(String* buffer) {
  ...
  if (command == "t1" || command == "t2" || command == "t3" || command == "s") {
...
void setup() {
  ...
  // It is fine if only one or two physical servos/plotters are connected; any
  // unused pin will simply have no load attached.
  for (uint8_t plotterIndex = 1; plotterIndex <= PLOTTER_COUNT; ++plotterIndex) {
    servos[plotterIndex - 1].attach(SERVO_PINS[plotterIndex - 1]);
    moveServo(plotterIndex, MARKER_ANGLE);
  }

  Serial.println("READY triple_toggle_servo_controller");
  Serial.println("Send t1, t2, or t3 to toggle plotter 1, 2, or 3.");
  printStatus();
}
